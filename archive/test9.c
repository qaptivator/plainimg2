...
int main(int argc, char* argv[]) {
    ...
    while (!quit) {
        ...
        if (state.keepAspectRatio) {
            ...
            int _scale = min(_windowWidth / _textureWidth, _windowHeight / _textureHeight);

            SDL_Log("SDL3 render SCALE: %d", _scale);

            // i should use SDL_FRect instead of SDL_Rect
            SDL_FRect destRect;

            destRect.w = _textureWidth * _scale;
            destRect.h = _textureHeight * _scale;
